...
try:
    from gmpy2 import mpz; mpzv, inttypes = 2, (int, long, type(mpz(1)))
except ImportError:
    try:
        from gmpy import mpz; mpzv, inttypes = 1, (int, long, type(mpz(1)))
    except ImportError:
        mpz, mpzv, inttypes = int, 0, (int, long)
...
if mpzv == 1: from gmpy import gcdfa
...
def isprime(n, tb=(3, 5, 7, 11), eb=(2,), mrb=()):  # TODO: more streamlining
    ...
    if n % 2 == 0 or n < 13 or n == isqrt(n) ** 2: return n in (
    2, 3, 5, 7, 11)  # Remove evens, squares, and numbers less than 13
...
def ecm(n, B1=10,
        B2=20):  # TODO: Determine the best defaults for B1 and B2 and the best way to increment them and iters
                ...
                for v in sums:
                    if v > thresh:
                        ...
                        for p in prime:
                            while sieve_val % p == 0:
                                if p in vec: sqr += [
                                    p]  # track perfect sqr facs to avoid sqrting something huge at the end
                                vec ^= {p}
            ...
            for col in xrange(num_smooth):
                ...
                for row in xrange(col + 1 - offset, num_used_prime):
                    if bitfields[row] & masks[col]:
                        if pivot:
                            bitfields[col - offset], bitfields[row], pivot = bitfields[row], bitfields[
                                col - offset], False
                        else:
...
def multifactor(n, methods=(pollardRho_brent, pollard_pm1, williams_pp1, ecm, mpqs), verbose=False):
    ...
    if verbose:
        ...
        print
        "\033[1;31m" + (names[g] if g in names else g) + "\033[;m",
        stdout.flush()
    return f
...
for i in range(1, (10**5)*5):
    # tan = 1
    tank = Counter(list(primefac(i)))
print(datetime.now()-st)
for i in range(1, (n//2)+2):
    prime_factors = Counter(list(primefac(i)))
    # print(i)
    # print(word)
    min_k = 1
    # if i == 27:
    #     pass
    for pr_f, pw in prime_factors.items():
        if pw % 2 != 0:
            min_k *= pr_f
        # if pw % 2 != 0:
        #     min_k *= pr_f**pw
    ...
    while min_k <= n:
        ...
        if j <= n and k <= n:
            if word[j] == 'b':
                if ((i, k) not in saved and (k, i) not in saved) and ((word[i] == 'a' and word[k] == 'c') or (word[i] == 'c' and word[k] == 'a')):
                    # print
                    saved.add((i, k))
                    saved.add((k, i))
                    # print(i, k, j)
            ...
            break
        idx += 1
        # print('min_k={}, i={}'.format(min_k, i))
        # print(min_k, i)
        # print(math.sqrt(min_k*i))
        # print(word[min_k])
        # if word[min_k] == 'a':
        #     if word[i] == 'c':
        #         print(min_k*i)
